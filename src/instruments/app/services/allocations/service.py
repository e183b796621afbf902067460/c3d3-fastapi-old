...
class AllocationService:
    ...
    def _get_allocation_by_pool_id(self, pool_id: int):
        allocation = self._session.query(
            ...
            base.SatelliteAddressesProtocolsCategoriesLabelsChains
        ).join(
            base.LinkAddressesProtocolsCategoriesChains,
            base.SatelliteAddressesProtocolsCategoriesLabelsChains.l_address_protocol_category_chain_id == base.LinkAddressesProtocolsCategoriesChains.l_address_protocol_category_chain_id
        ).join(
    ...
    def on_post__allocations_add_allocation(
            ...
            protocol: str,
            protocol_category: str
    ) -> Optional[schemas.allocations.AllocationORMSchema]:
        ...
        allocation = self._get_allocation_by_pool_id(pool_id=pool.l_address_protocol_category_chain_id)
        if not allocation:
            allocation = base.SatelliteAddressesProtocolsCategoriesLabelsChains(
                l_address_protocol_category_chain_id=pool.l_address_protocol_category_chain_id
            )
            self._session.add(allocation)
            self._session.commit()
            return schemas.allocations.AllocationORMSchema.from_orm(self._get_allocation_by_pool_id(pool_id=pool.l_address_protocol_category_chain_id))
        return None
    ...
    def on_delete__allocations_delete_allocation(
        ...
        allocation = self._get_allocation_by_pool_id(pool_id=pool.l_address_protocol_category_chain_id)
        if allocation:
            self._session.query(base.SatelliteAddressesProtocolsCategoriesLabelsChains).filter_by(l_address_protocol_category_chain_id=pool.l_address_protocol_category_chain_id).delete()
            self._session.commit()
            return True
        return False
