...
class TestPoolCBV:
    ...
    def test_on_post__pools_add_pool(
            self,
            client, jwt_token,
            h_protocols, h_protocols_categories,
            l_protocols_categories, l_protocols_categories_chains,
        ...
        for wallet in wallets:
            for pool in pools:
                response = client.post(f"{settings.API_V1}/instruments/pools/{wallet['h_network_name']}/{wallet['h_address']}", content=pool, headers=jwt_token)
    ...
    def test_on_delete__pools_delete_pool(
            self,
            client, jwt_token,
            h_protocols, h_protocols_categories,
            l_protocols_categories, l_protocols_categories_chains,
        ...
        for wallet in response.json():
            for pool in pools:
                ...
                response = client.delete(
                    f"{settings.API_V1}/instruments/pools/"
